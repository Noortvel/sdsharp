...
namespace StableDiffusion.ML.OnnxRuntime
{
    public class UNet
    {
        ...
        public static List<NamedOnnxValue> CreateOpenposeControlNetModelInput(
            Tensor<float> encoderHiddenStates,
            Tensor<float> sample,
        ...
        public static SixLabors.ImageSharp.Image Inference(String prompt, StableDiffusionConfig config)
        {
            ...
            for (int t = 0; t < timesteps.Length; t++)
            {
                ...
                //input = CreateOpenposeControlNetModelInput(textEmbeddings, latentModelInput, timesteps[t]);
        ...
        public static SixLabors.ImageSharp.Image InferenceControllNet(
            string prompt,
            StableDiffusionConfig config)
        {
            ...
            var sessionOptions = config.GetSessionOptionsForEp();
            // Create Inference Session
            var unetSession = new InferenceSession(config.OpenPoseControllnetOnnxPath, sessionOptions);
            ...
            for (int t = 0; t < timesteps.Length; t++)
            {
                ...
                Console.WriteLine($"scaled model input {latentModelInput[0]} at step {t}. Max {latentModelInput.Max()} Min{latentModelInput.Min()}");
                //input = CreateUnetModelInput(textEmbeddings, latentModelInput, timesteps[t]);
                input = CreateOpenposeControlNetModelInput(
                    textEmbeddings,
                    latentModelInput,
                    latentModelInput,
                    timesteps[t]);
            ...
            PrintArray(latents.ToArray());
            PrintArray(latents.Dimensions.ToArray());
        ...
        private static void PrintArray<T>(T[] values)
        {
            var content = string.Join(", ", values.Select(x => x.ToString()));
