...
namespace ControlnetApp
{
    public class TensorHelper
    {
        ...
        public static Tuple<Tensor<TTensorType>, Tensor<TTensorType>> SplitTensor<TTensorType>(
            Tensor<TTensorType> tensorToSplit,
            int sections)
        {
        ...
        public static Tensor<float> PrependZeros(Tensor<float> original)
        {
            var dims = original.Dimensions.ToArray();
            dims[0] *= 2;

            var result = new DenseTensor<float>(dims);
            for(int i = 0; i < original.Length; i++)
            {
                int index = (int)original.Length + i;
                result.SetValue(i, 0); //Zeros to start.
                result.SetValue(index, original.GetValue(i));
            }
